...
from modulollmdados.llmconnection import postgre_connection
from modulollmdados.llm_get_dados import llm_extrai_dados

# Função para gerar um ID único para cada documento com base no conteúdo
def llm_gera_id_documento(doc):
...
# Função para criar a tabela 'llm_documentos' no banco de dados
def llm_cria_tabela():
...
# Função para inserir dados JSON na tabela 'llm_documentos'
def llm_insere_dados_json():

    # Inicializa uma lista para armazenar os dados a serem inseridos
    allData = []
    
    # Carrega os dados JSONL do arquivo especificado
    llmDados = orjsonl.load(f"{os.getcwd()}/dags/dados/dataset1.jsonl")
    ...
    insert_query = """
    INSERT INTO llm_documentos (doc_id, question, answer) VALUES (%s, %s, %s)
    """

    # Processa cada entrada no JSON para os primeiros 25 registros
    for data in llmDados[0:25]:
        ...
        docId = llm_gera_id_documento(data)
    ...
    try:
        ...
        args = ','.join(cur.mogrify("(%s,%s,%s)", i).decode('utf-8')
                        for i in allData)
        insert_query = "INSERT INTO llm_documentos (doc_id, question, answer) VALUES" + (args)
...
# Função para inserir dados CSV na tabela 'llm_documentos'
def llm_insere_dados_csv():
    ...
    llmDados = pd.read_csv(f"{os.getcwd()}/dags/dados/dataset2.csv")

    # Processa as primeiras 25 linhas do CSV
    for index, row in llmDados.head(25).iterrows():
...
def llm_cria_indice():
    ...
    esClient = Elasticsearch("http://a7b0726a95d8:9200")
